<h1 align="center">Tanish Jain</h1>
<p align="center"><b>Backend Engineer • Distributed Systems • DevOps</b></p>

<p align="center">
  <a href="https://github.com/05tanish">
    <img src="https://img.shields.io/badge/GitHub-05tanish-black?style=flat-square&logo=github">
  </a>
  <a href="https://linkedin.com/in/YOUR_LINKEDIN">
    <img src="https://img.shields.io/badge/LinkedIn-blue?style=flat-square&logo=linkedin">
  </a>
  <a href="https://leetcode.com/YOUR_USERNAME">
    <img src="https://img.shields.io/badge/LeetCode-orange?style=flat-square&logo=leetcode">
  </a>
  <a href="mailto:tanishjain262@gmail.com">
    <img src="https://img.shields.io/badge/Email-red?style=flat-square&logo=gmail">
  </a>
</p>

---

## 🧠 Profile

Backend engineer building **scalable, fault-tolerant systems** with focus on **throughput, latency, and reliability**.
Experienced in **API design, async processing, caching, and observability-driven systems**.

---

## ⚙️ Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=js,ts,python,nodejs,express,django,postgres,mongodb,redis,docker,nginx,linux,githubactions,grafana,prometheus" />
</p>

---

## 📊 Engineering Signals

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=05tanish&show_icons=true&theme=tokyonight&hide_border=true" height="140"/>
  <img src="https://streak-stats.demolab.com/?user=05tanish&theme=tokyonight&hide_border=true" height="140"/>
</p>

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=05tanish&theme=tokyo-night"/>
</p>



## ⚙️ Engineering Workflow

```text id="6mmr34"
Design → API Contracts → Build → Optimize → Containerize → CI/CD → Deploy → Observe → Scale
```

---

## 🌐 Links

* LinkedIn →[linkden](https://www.linkedin.com/in/tanish-jain-5b986828a/)
* LeetCode → [leetcode](https://leetcode.com/u/tanish993678/)
* Email → [tanishjain262@gmail.com](mailto:tanishjain262@gmail.com)

---

## 🧠 Engineering Principle

> Systems should be designed for failure, optimized for performance, and observable in production.

---
